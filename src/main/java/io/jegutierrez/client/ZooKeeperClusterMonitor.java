...
public class ZooKeeperClusterMonitor {
    ...
    ZooKeeperClusterMonitor(ZooKeeper zk, DataKeeperClusterInfo clusterInfo) {
        ...
        zk.exists(String.format("/%s/%s", CLUSTER, LEADER), getLeaderElectionWatcher(), getLeaderRegisterCallback(zk),
                null);
    }
        ...
        return new StatCallback() {
            ...
            public void processResult(int rc, String path, Object ctx, Stat stat) {
                ...
                if (exists) {
                    ...
                    zk.create(path, clusterInfo.getNodeName().getBytes(), Ids.OPEN_ACL_UNSAFE, CreateMode.EPHEMERAL,
                            getGenericCreationCallback(zk), 0);
                }
                if (b != null) {
        ...
        return new StatCallback() {
            ...
            public void processResult(int rc, String path, Object ctx, Stat stat) {
                ...
                switch (Code.get(rc)) {
                    case CONNECTIONLOSS:
                        /*
                         * Handling connection loss for a sequential node is a bit delicate. Executing
                         * the ZooKeeper create command again might lead to duplicate Masters. For now,
                         * let's assume that it is ok to create a duplicate Master.
                         */
                        if (count <= 5) {
                            zk.create(path, clusterInfo.getNodeName().getBytes(), Ids.OPEN_ACL_UNSAFE,
                                    CreateMode.EPHEMERAL, getGenericCreationCallback(zk), count + 1);
